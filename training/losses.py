...
def dirichlet_loss(labels_for_q, concentrations_for_q):
    ...
    # print(total_count, concentrations_for_q)
...
def get_dirichlet_neg_log_prob(labels_for_q, total_count, concentrations_for_q):
    ...
    dist = pyro.distributions.DirichletMultinomial(total_count=total_count, concentration=concentrations_for_q,
                                                   is_sparse=False)
    return -dist.log_prob(labels_for_q)  # important minus sign

if __name__ == '__main__':
    get_dirichlet_neg_log_prob([12,1,1], 14, [5,3,2])